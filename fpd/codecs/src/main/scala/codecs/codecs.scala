  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(as) =>
        ...
        if decoded.forall(!_.isEmpty) then Option(decoded.flatten)
        else None
      case _ => None
    }
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    ...
    println(Decoder.field[String]("name").decode(maybeJsonObj2.get))

    val personJson = summon[Encoder[Person]].encode(Person("dd", 2))
    println(personJson)
    println(summon[Decoder[Person]].decode(personJson))

    val contactsJson = summon[Encoder[Contacts]].encode(Contacts(List(Person("dfe", 3), Person("feg", 6))))
    println(contactsJson)
    println(summon[Decoder[Contacts]].decode(contactsJson))
