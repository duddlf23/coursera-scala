...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] =
    val currentState = initial.head
    val newNeighbors = newNeighborsOnly(neighborsWithHistory(currentState._1, currentState._2), explored)
    val newInitial = initial.tail ++ newNeighbors
    val newExplored = explored ++ newNeighbors.map(_._1).toSet
    currentState #:: from(newInitial, newExplored)
