...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.aggregate(0)(
    (agg, article) => if (article.mentionsLanguage(lang)) agg + 1 else agg, _ + _ )
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap(article => langs.filter(article.mentionsLanguage).map(l => (l, article))).groupByKey()
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.map(x => (x._1, x._2.toList.length)).collect.sortWith(_._2 > _._2).toList
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    ...
    rdd.flatMap(article => langs.filter(article.mentionsLanguage).map(l => (l, 1))).reduceByKey(_ + _).sortBy(_._2, false).collect.toList
