...
//    timeUsageGroupedSql(summaryDf).show()
  ...
  def row(line: List[String]): Row =
    Row(
      line.head :: line.tail.map(_.toDouble): _*
    )
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryColsPrefix = List("t01", "t03", "t11", "t1801", "t1803")
    val workingColsPrefix = List("t05", "t1805")
    val otherColsPrefix = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")


    def targetCols(targetCategory: List[String], additionalCond: String => Boolean = _ => true): List[Column] = {
      for {
        colName <- columnNames if targetCategory.exists(colName.startsWith) && additionalCond(colName)
      } yield new Column(colName)
    }

    val pr = targetCols(primaryColsPrefix)
    val working = targetCols(workingColsPrefix)
    val other = targetCols(
      otherColsPrefix,
      colName => !(primaryColsPrefix ++ workingColsPrefix).exists(colName.startsWith)
    )
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column = {
      when(col("telfs").between(1, 2), "working")
        .otherwise("not working").as("working")
    }
    val sexProjection: Column = {
      when(col("tesex") === 1, "male").otherwise("female").as("sex")
    }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val roundedDS = groupedDS
      .select(
        'working,
        'sex,
        'age,
        round('primaryNeeds, 1).as[Double].as("primaryNeeds"),
        round('work, 1).as[Double].as("work"),
        round('other, 1).as[Double].as("other"),
      ).as[TimeUsageRow]
